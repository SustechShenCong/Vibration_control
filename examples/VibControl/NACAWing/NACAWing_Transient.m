function varargout = NACAWing_Transient(obj,Omega,varargin)
...
N = obj.N;
[integrationMethod, nCycles, zInit, outdof, nSteps, Plot, samp, ts] =...
...
%         odeoptions = odeset('RelTol',1e-9,'AbsTol',1e-9);
        [t0, x0] = ode45(odefun, linspace(ts,ts+T,nSteps*nCycles+1), zInit);

    case 'ode15s'
%         odeoptions = odeset('RelTol',1e-9,'AbsTol',1e-9);
        [t0, x0] = ode15s(odefun, linspace(ts,ts+T,nSteps*nCycles+1), zInit);
    ...
    case {'Newmark', 'Galpha'}
        ...
        if strcmp(integrationMethod,'Newmark')
            TI = ImplicitNewmark('timestep',h,'alpha',0.005,'t0',ts);
        elseif strcmp(integrationMethod,'Galpha')
            TI = GeneralizedAlpha('timestep',h,'rho_inf',0.7,'t0',ts);
        end
        % Residual evaluation function handle            
        q0 = zInit(1:n); qd0 = zInit(n+1:N); 
        qdd0 = obj.M\(obj.compute_fext(ts) - obj.C*qd0 - obj.K*q0 ...
            - obj.compute_fnl(q0,qd0));

        % Nonlinear Time Integration
        TI.Integrate(q0,qd0,qdd0,ts+T,residual);
...
function [integrationMethod,nCycles,zInit,outdof,nSteps,Plot,samp,ts] = parse_inputs(N,varargin)
%% parsing inputs
